...
DOCS_PATH = Path(__file__).parent.parent / "docs" / "EVAL_RESULTS.md"
...
def _write_docs(rows: list[dict], n_passed: int) -> None:
    """Human-readable companion to last_run_results.json (which has the full
    per-check reasoning but isn't committed). Regenerated by the eval-docs CI
    job whenever backend/llm/, backend/rag/, backend/knowledge_base/, or
    evals/ change -- see .github/workflows/ci.yml."""
    lines = [
        "# Eval Results",
        "",
        f"_Last updated: {dt.datetime.now(dt.timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} — {n_passed}/{len(rows)} test cases passed._",
        "",
    ...
    for row in rows:
        if "error" in row:
            lines.append(f"| {row['id']} | {row['category']} | ERROR | {row['error'][:100]} |")
            continue
        result = "PASS" if _row_passed(row) else "FAIL"
        lines.append(f"| {row['id']} | {row['category']} | {result} | {', '.join(_row_notes(row))} |")
    lines += [
        "",
        "Full results, including each check's reasoning, are written to `evals/last_run_results.json` "
        "(not committed — regenerate with `python -m evals.run_evaluation`).",
    ]
...
def main() -> None:
    ...
    for row in rows:
        ...
        n_passed += int(passed)
        print(f"{row['id']:16s} {row['category']:14s} {('PASS' if passed else 'FAIL'):6s} {', '.join(_row_notes(row))}")
    print("-" * 100)
    print(f"{n_passed}/{len(rows)} test cases passed. Full results (incl. reasoning) written to {RESULTS_PATH}")

    _write_docs(rows, n_passed)
    print(f"Human-readable summary written to {DOCS_PATH}")
